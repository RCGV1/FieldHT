...
enum AdvancedFrequencyScanTests {
    static func main() throws {
        ...
        if let scan = try? String(contentsOf: scanURL, encoding: .utf8) {
            ...
            expect(!scan.contains("toneSummary"), "recent scan hits must not claim to detect unsupported tones")
            expect(!scan.contains("Receive Tone"), "saving a scan hit must not present an unverified receive tone")
            expect(!scan.contains("Transmit Tone"), "saving a scan hit must not present an unverified transmit tone")
