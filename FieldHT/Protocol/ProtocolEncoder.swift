...
public struct ProtocolEncoder {
    ...
    public static func encodeSettings(_ settings: Settings) -> Data {
        ...
        stream.writeInt(settings.kissTxDelay, bitCount: 8)
        stream.writeInt(settings.kissTxTail, bitCount: 8)
        stream.writeBool(settings.voxEnabled)
        stream.writeInt(settings.voxLevel, bitCount: 3)
        ...
        stream.writeBytes(settings.rawExtensionData)
